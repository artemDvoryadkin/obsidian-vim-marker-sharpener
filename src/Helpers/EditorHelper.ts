...
export class EditorHelper {
	...
	get editor(): Editor {
		...
		return this._editor || this.getEditorOrThrow();
	}

	private getEditorOrThrow(): Editor {
		if (!this._editor) {
	...
	replaseLine(lineText: string, lineNumber: number) {
		...
		editor.replaceRange(lineText, { line: lineNumber, ch: 0 }, { line: lineNumber, ch: line.length })
	}
	...
	replaseLines(linesText: string[], fromLineNumber: number, toLineNumber: number) {
		...
		editor.replaceRange(textLines, { line: fromLineNumber, ch: 0 }, { line: toLineNumber, ch: lenLastLine })
	}
	...
	getLineByCursor(): string {
		return this.editor.getLine(this.editor.getCursor().line)
	}
